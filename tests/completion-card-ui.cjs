...
(async()=>{
 ...
 for(const [width,height] of [[375,667],[390,844],[360,800],[1280,900]])for(const colorScheme of ['light','dark']){
  ...
  assert.equal(m.modal,width<681);assert.equal(m.overflow,false);assert.ok(m.card.x>=0&&m.card.y>=0&&m.card.right<=width&&m.card.bottom<=height);assert.ok(m.support.height>=44&&m.feedback.height>=44);assert.equal(m.img.height,m.lineHeight);assert.equal(m.gap,'4px');assert.ok(m.copy.includes('DJI pocket 4P'));assert.ok(m.copy.includes('What goes around comes around!'));assert.ok(!m.copy.includes('Sony'));assert.equal(m.amount,'$459 raised · Goal: $— (pending confirmation)');
