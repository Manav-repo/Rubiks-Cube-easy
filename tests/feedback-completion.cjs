...
function setup(initial=false,stamp=null){
 ...
 for(const id of ['camera-fund-amount','completion-fund-amount','camera-fund-progress','completion-fund-progress','feedback-dialog','feedback-open','feedback-form','feedback-message','feedback-close','completion-dialog','completion-close','completion-feedback','solution-caption','playback-speed']){
  nodes[id]={textContent:'',disabled:false,listeners:{},isConnected:true,addEventListener(k,f){this.listeners[k]=f},setAttribute(){},focus(){},showModal(){this.opens=(this.opens||0)+1;open=true},close(){open=false;this.listeners.close?.()},setCustomValidity(v){this.error=v},reportValidity(){return !nodes['feedback-message'].error}};
 }
 ...
 vm.runInNewContext(fs.readFileSync('dist/js/feedback.js','utf8'),{document,window,encodeURIComponent,localStorage:{getItem:()=>stamp,setItem(k,v){stamp=v}},MutationObserver:class{constructor(f){observer=f}observe(){}},setTimeout(f,ms){assert.equal(ms,3000);timers.set(++seq,f);return seq},clearTimeout(id){timers.delete(id)}});
...
assert.equal(a.nodes['completion-fund-progress'].value,459);assert.equal(a.nodes['camera-fund-progress'].max,1000);
