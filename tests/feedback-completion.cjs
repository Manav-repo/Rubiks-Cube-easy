...
function setup(initial=false){
 ...
 vm.runInNewContext(fs.readFileSync('dist/js/feedback.js','utf8'),{document,window,encodeURIComponent,sessionStorage:{getItem:()=>null,setItem(){}},MutationObserver:class{constructor(f){observer=f}observe(){}},setTimeout(f,ms){assert.equal(ms,3000);timers.set(++seq,f);return seq},clearTimeout(id){timers.delete(id)}});
