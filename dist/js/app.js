...
function render(){
 ...
 syncThemeButton();
 document.body.classList.toggle('fix',state.mode==='fix');document.body.classList.toggle('playback',state.mode==='fix'&&state.solution.length>0&&state.playback);document.body.classList.toggle('symbols',state.symbols||state.accessible);document.body.classList.toggle('accessible',state.accessible);
...
function setupCalibration(){const dialog=$('calibration');$('layout-choices').innerHTML=['Red','Orange'].map(c=>`<button type="button" data-layout="${c.toLowerCase()}-right" aria-label="${c} is on my right">${cornerDiagram(c)}<strong>${c} on my right</strong></button>`).join('');document.querySelectorAll('[data-layout]').forEach(b=>b.onclick=()=>{const changed=state.layout!==b.dataset.layout;if(changed){invalidateSolution();state.paint=CubeLayout.blank();}state.layout=b.dataset.layout;state.calibrationSkipped=false;dialog.close();render();if(pendingSolve){pendingSolve=false;$('solver-message').textContent=changed?'Layout matched. Paint your cube, then find your moves.':'';if(!changed)$('solve').click();}});$('calibrate-settings').onclick=()=>{if(!animating&&!planning)openCalibration();};$('calibration-skip').onclick=()=>{state.calibrationSkipped=true;pendingSolve=false;dialog.close();save();};dialog.addEventListener('cancel',()=>{state.calibrationSkipped=true;pendingSolve=false;save();});}
function openCalibration(){$('settings').hidden=true;$('settings-toggle').setAttribute('aria-expanded','false');$('calibration-note').textContent='Changing the layout starts a fresh painting. Your lesson badges stay.';if(!$('calibration').open)$('calibration').showModal();}
